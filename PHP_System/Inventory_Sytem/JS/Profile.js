document.addEventListener('DOMContentLoaded', () => {
            ...
            fetch('../PHP/Profile.php')
                ...
                .then(data => {
                    if (profilePicturePreview) {
                        profilePicturePreview.src = data.profile_picture || '/Pictures/blank-profile-picture.png';
                    }
                });
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        if (data.profile && data.profile.profile_picture) {
                            sidebarProfilePicture.src = data.profile.profile_picture;
                            profilePagePicture.src = data.profile.profile_picture;
                        } else {
                            console.error('Profile picture is missing in the response.');
                        }
    ...
    if (personalInfoEditButton && personalInfoPopup) {
        personalInfoEditButton.addEventListener('click', () => {
            personalInfoPopup.classList.remove('hidden');
        });
    }
    if (cancelPersonalInfoButton && personalInfoPopup) {
        cancelPersonalInfoButton.addEventListener('click', () => {
            personalInfoPopup.classList.add('hidden');
        });
    }
    if (personalInfoForm && personalInfoPopup) {
        personalInfoForm.addEventListener('submit', (e) => {
            ...
            .catch(() => {
                alert('An error occurred while updating your personal info.');
            });
        });
    }
    ...
    // Show popup and autofill fields
    ...
    // Fetch the profile picture URL from the session or backend
    fetch('../PHP/Profile.php')
        .then(response => response.json())
        .then(data => {
            const profilePictureUrl = data.profile_picture || '/Pictures/blank-profile-picture.png';

            // Update all profile picture elements
            const profileImages = document.querySelectorAll('img[src="/Pictures/blank-profile-picture.png"]');
