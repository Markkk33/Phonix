document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchProductList() {
        return fetch('../PHP/Products.php?action=fetch')
            .then(res => res.json())
            .then(data => {
                // Only include products that are NOT out of stock
                productList = Array.isArray(data) ? data.filter(p => p.status !== 'Out of Stock') : [];
            });
    }

    // --- Fetch Orders from Backend ---
    function fetchOrders() {
        return fetch('../PHP/Orders.php?action=fetch')
            .then(res => res.json())
            .then(data => {
                allOrders = Array.isArray(data) ? data.map(row => ({
                    id: row.id,
                    orderId: row.order_id,
                    product: row.product,
                    productId: row.product_id || null,
                    img: row.img ? row.img : '/Pictures/phonIX.png',
    ...
    function showEditOrderPopup(idx) {
      ...
      // Fetch product list and update dropdown (exclude out of stock, but include current product)
      fetchProductList().then(() => {
        const productSelect = document.getElementById('edit-order-product');
        // Display the product name as plain text and preserve product_id in a hidden input
        const currentProduct = productList.find(p => String(p.id) === String(order.productId));
        const productName = currentProduct ? currentProduct.name : order.product;
        console.log('Current product:', currentProduct); // Debug log
        productSelect.outerHTML = `
          <span id="edit-order-product" class="product-name-display">${productName}</span>
          <input type="hidden" id="edit-order-product-id" value="${order.productId}">
        `;
        console.log('Product ID set in hidden input:', order.productId); // Debug log

        // Ensure product_id is set correctly
        if (!order.productId || isNaN(order.productId)) {
          console.warn('Invalid or missing productId for order:', order);
          document.getElementById('edit-order-product-id').value = '';
        } else {
        ...
        function updateAmount() {
          let price = currentProduct ? currentProduct.price : (order.price || null);
          if (!price && order.amount && order.quantity) {
            let amt = parseFloat(String(order.amount).replace('$',''));
            let qty = parseInt(order.quantity, 10);
            if (qty > 0) price = (amt / qty).toFixed(2);
          }
          const qty = parseInt(document.getElementById('edit-order-quantity').value, 10) || 0;
          if (price && qty > 0) {
            document.getElementById('edit-order-amount').value = `$${(qty * parseFloat(price)).toFixed(2)}`;
          } else {
            document.getElementById('edit-order-amount').value = '';
          }
        }
        document.getElementById('edit-order-quantity').oninput = updateAmount;
        // Set initial amount
        updateAmount();
    ...
    document.getElementById('edit-order-form').onsubmit = function (e) {
      ...
      const id = order.id;
      const productId = parseInt(document.getElementById('edit-order-product-id').value, 10); // Ensure product_id is an integer
      console.log('Product ID sent to backend:', productId); // Debug log
      if (isNaN(productId) || productId <= 0) {
        showOrderNoticePopup('Invalid product selected.');
        return;
      }
      ...
      if (isNaN(quantity) || quantity <= 0) {
        showOrderNoticePopup('Invalid quantity.');
        return;
      }
      const payment = document.getElementById('edit-order-payment').value;
      const status = document.getElementById('edit-order-status').value;
      console.log('Form data being sent:', { id, productId, customer, quantity, payment, status }); // Debug log
      const formData = new FormData();
    ...
    if (addOrderBtn) {
        addOrderBtn.addEventListener('click', async function () {
            ...
            if (!addOrderPopup) {
                ...
                // Product dropdown (exclude out of stock)
                let productOptions = productList.map(p => `<option value="${p.id}" data-price="${p.price}">${p.name} (${p.storage}, ${p.color})</option>`).join('');
                ...
                <div class="popup-content">
                    ...
                    <form id="add-order-form" class="popup-body">
                        <div class="form-group">
                            <label>Product:</label>
                            <select id="add-order-product" required>${productOptions}</select>
                        </div>
                        <div class="form-group">
                ...
                // Update product dropdown (exclude out of stock)
                let productOptions = productList.map(p => `<option value="${p.id}" data-price="${p.price}">${p.name} (${p.storage}, ${p.color})</option>`).join('');
                addOrderPopup.querySelector('#add-order-product').innerHTML = productOptions;
            }
            ...
            function updateAmount() {
                const selected = productList.find(p => String(p.id) === productSelect.value);
            ...
            updateAmount();
            // Ensure only one event listener for cancel button
            ...
            // Ensure only one event listener for form submit
