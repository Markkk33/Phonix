document.addEventListener('DOMContentLoaded', function () {
    ...
    const BACKEND_BASE_URL = '/api/products';
    ...
    // Update the add product form submission logic
    ...
    addProductForm.onsubmit = function (event) {
        ...
        .then(data => {
            if (data.success) {
                showNoticePopup('Product added successfully!');
                addProductPopup.classList.add('hidden'); // Hide the popup
                // Refresh the products table
                fetchProducts();
            } else {
                showNoticePopup(data.message || 'Failed to add product.');
            }
        })
        .catch(() => {
        ...
        fetch('/get_products')
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showNoticePopup('Failed to fetch products.');
                }
            })
            .catch(error => {
    ...
    if (editProductForm) {
        editProductForm.onsubmit = function(e) {
            ...
            .then(data => {
                if (data.success) {
                    showNoticePopup('Product updated successfully!');
                    editProductPopup.classList.add('hidden');
                    fetchProducts(); // Refresh the product table and preview
                } else {
                    showNoticePopup(data.message || 'Failed to update product.');
                }
            })
            .catch(error => {
                console.error('Error updating product:', error);
                showNoticePopup('An error occurred while updating the product.');
            });
        };
    }
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            console.log('Search query:', query); // Debugging log for search query

            if (query === '') {
                filteredProducts = null;
            } else {
                filteredProducts = allProducts.filter(product => {
                    const id = String(product.id).toLowerCase(); // Ensure id is a string
                    const name = product.name.toLowerCase();
                    const color = product.color.toLowerCase();
                    const storage = product.storage.toLowerCase();

                    return id.includes(query) || name.includes(query) || color.includes(query) || storage.includes(query);
                });
            }

            console.log('Filtered products:', filteredProducts); // Debugging log for filtered results
            currentPage = 1;
            renderTable(currentPage);
            updatePagination();
        });
    ...
    if (productsHeaderControls) {
        ...
        // Fetch available storage and color options from backend
        fetch('/get_filter_options')
            .then(res => res.json())
            .then(data => {
                // Populate storage dropdown
                if (data.success && data.storageOptions) {
                    storageSelect.innerHTML = '<option value="All">All Storage</option>' +
                        data.storageOptions.map(storage => `<option value="${storage}">${storage}</option>`).join('');
                }

                // Populate color dropdown
                if (data.success && data.colorOptions) {
                    colorSelect.innerHTML = '<option value="All">All Colors</option>' +
                        data.colorOptions.map(color => `<option value="${color}">${color}</option>`).join('');
                }
            })
            .catch(error => {
        ...
        function updateFilterDropdowns() {
            fetch(`${BACKEND_BASE_URL}/options`)
                .then(res => res.json())
                .then(data => {
    ...
    function sendEditRequest(formData) {
        ...
        .then(data => {
            if (data.success) {
                ...
                product.price = `$${parseFloat(formData.price).toFixed(2)}`;
                product.status = formData.stock === 0 ? 'Out of Stock' : (formData.stock <= 5 ? 'Low Stock' : 'In stock');
                editProductPopup.classList.add('hidden');
                renderTable(currentPage);
                updatePagination();
