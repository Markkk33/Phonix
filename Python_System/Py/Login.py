...
def profile():
    ...
    if request.method == 'POST':
        ...
        if 'profile_picture' in request.files:
            ...
            if file and file.filename:
                filename = secure_filename(f'user_{user_id}_profile_{file.filename}')
                ...
                try:
                    ...
                    flash("An error occurred while saving your profile picture.", "error")
        ...
        try:
            ...
            logging.error(f"Error while saving profile: {e}")
    ...
    logging.debug(f"Is profile incomplete: {is_profile_incomplete}")
...
def api_profile():
    ...
    incomplete = not profile_data or any(
        not profile_data.get(field) for field in required_fields
    )
